...
func parseSession(block []line) (Session, error) {
	...
	if err != nil {
		return Session{}, NewError(UndefinedDate, headline.Number)
	}
	...
	for i, field := range fields {
		...
		if strings.HasPrefix(field, "[") != strings.HasSuffix(field, "]") {
			return Session{}, NewError(InvalidReward, headline.Number)
		}
		...
		if reward != nil {
			return Session{}, NewError(RewardAlreadyFound, headline.Number)
		}

		// Check position of the reward
		if i != 0 && i != len(fields)-1 {
			return Session{}, NewError(WrongRewardPosition, headline.Number)
		}
